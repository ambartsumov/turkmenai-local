...
| Model catalog | **Partial** | Models are discovered live from the Hugging Face Hub, grouped by specialization and ranked against the local hardware (RAM/VRAM/disk); incompatible models are hidden behind an explicit view. A small embedded starter catalog + local cache serve offline/blocked networks. One-click install (wiring the catalog to the resumable downloader and runtime) is not yet complete — the card links to the Hugging Face source. |
| Dataset catalog | **Partial** | Datasets are discovered from the Hugging Face Hub by specialization with license/risk flags and checked against free disk. The Data Inspector and controlled local fine-tuning (LoRA/QLoRA) are not implemented yet. |
