...
| First-run wizard | **Not available** | The native console can show hardware, but the full beginner wizard and automatic recommended-model installation are not implemented. |
| Inference | **Not available** | No verified runtime and model are bundled. `/v1/chat/completions` correctly returns `NO_ACTIVE_RUNTIME`. |
...
| Local API | **Partial** | Loopback health, hardware, analysis, plan and capability endpoints are verified. Inference requires the unimplemented verified-runtime integration. |
