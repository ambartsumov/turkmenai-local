...
export default function Home() {
  ...
  return <main className="site-shell">
    ...
    <footer><div className="footer-brand"><SignalLogo /><span>TurkmenAI <em>Local</em></span></div><p>{t.footer}</p><p className="footer-license">{t.license}</p><p className="footer-diagnostics">v{product.version} · {product.channel} · <a href={`${product.repository}/issues/new?labels=bug&title=${encodeURIComponent(`[bug] v${product.version} — `)}&body=${encodeURIComponent(`\n\n---\n${product.product} v${product.version} (${product.channel})\nUA: ${typeof navigator !== "undefined" ? navigator.userAgent : ""}`)}`} target="_blank" rel="noreferrer">{t.reportBug} <ArrowUpRight size={12} /></a></p></footer>
  </main>;
}
